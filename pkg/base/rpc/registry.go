...
func (rg *ServiceRegistry) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		...
		err = qs.NewDecoder().Decode(reqVal.Interface(), r.URL.Query())
		if err != nil {
			if base.IsDev {
	...
	if errIface != nil {
		err = errIface.(error)
		log.WithField("ReqID", rID).WithField("Error", fmt.Sprint(err)).Warn("call-service error")
	}
